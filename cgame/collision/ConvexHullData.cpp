...
bool CConvexHullData::LoadBinaryData(FILE* InFile)
{
	...
	//读出标志，从而检验！
	const char *szCHDataFlag=CHDATA_HEAD_TAG;
	char szRead[60];
	memset(szRead,0,60);
	...
	case 1:
		//读出标志位
		fread(&m_dwFlags, sizeof(unsigned int), 1, InFile);
		break;
	case 0:
		m_dwFlags=0;
		break;
...
#ifdef BMAN_EDITOR
void CConvexHullData::Render(A3DFlatCollector* pFC,  bool bRenderV,CFace* pSpecialFace, unsigned int dwVColor,unsigned int dwFColor, const A3DVECTOR3& vRayDir, const A3DMATRIX4* pTransMatrix)const
{
	if(!pFC) return;
	...
	unsigned int ALPHA=dwFColor & 0xff000000;
