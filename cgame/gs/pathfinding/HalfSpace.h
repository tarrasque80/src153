...
// ****** Noted by wenfeng,05-10-25
// Since this file will run in the server,so here we only include "A3DVector.h"
...
public:
	...
	void Project(const A3DVECTOR3& vPos, A3DVECTOR3& vProjPos) const
	{
		vProjPos = vPos - DistV2Plane(vPos) * m_vNormal;
	}

	// Mirror a 3D position refer to me
	void Mirror(const A3DVECTOR3& vPos, A3DVECTOR3& vMirrorPos) const
	{
		vMirrorPos = vPos - 2 * DistV2Plane(vPos) * m_vNormal;
	}

	void Mirror(A3DVECTOR3& vPos) const
	{
		vPos = vPos - 2 * DistV2Plane(vPos) * m_vNormal;
	}
