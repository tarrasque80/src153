...
template <class _Key, class _Value,unsigned int map_cap>
class static_map
{
public:
...
private:
	...
	static_set<_Value,_Allocator> & operator=(const static_set<_Value,_Allocator> &) ;
public:
	static_set()
	{} 
...
template <unsigned int _MaxBitSize = 65536, typename _Allocator = default_alloc>
class bitmap
{
private:
...
public:
	...
	const unsigned char * data(unsigned int &size) const 
	{
		size = _data.size();
		return _data.begin();
	}
	...
	void init(unsigned char * buf, unsigned int size)
	{
		_data.clear();
