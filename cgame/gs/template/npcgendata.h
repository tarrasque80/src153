...
{
	unsigned int	dwID;			//	NPC ID
	unsigned int	dwNum;			//	NPC number
	int		iRefresh;		//	Refresh time
	unsigned int	dwDiedTimes;	//	Time of death before new property generatored
	unsigned int	dwAggressive;	//	Aggressive monster
	float	fOffsetWater;	//	Offset to water face
	float	fOffsetTrn;		//	Offset to terrain surface
	
	unsigned int	dwFaction;		//	怪物自己的类别
	unsigned int	dwFacHelper;	//	怪物寻求帮助的类别
	unsigned int	dwFacAccept;	//	怪物响应帮助的类别
...
struct NPCGENFILERES
{
	...
	unsigned int	dwRefreshTime;	//	Refresh time
	unsigned int	dwNumber;		//	Resource number
...
{
	unsigned int	dwDynObjID;		//	Dynamic object ID
	float	vPos[3];
	unsigned char dir[2];
	unsigned char rad;
};
