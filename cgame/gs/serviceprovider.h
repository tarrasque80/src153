...
private:
	...
	virtual bool OnInit(const void * buf, unsigned int size) = 0;
	virtual void GetContent(const XID & player,int cs_index,int sid) = 0;
	virtual void TryServe(const XID & player, const void * buf, unsigned int size)=0;
	virtual void OnHeartbeat() = 0;
	virtual void OnControl(int param ,const void * buf, unsigned int size) = 0;
public:
...
public:
	...
	bool Init(gactive_imp * imp,const void * buf, unsigned int size)
	{
	...
	void PayService(const XID & player,const void * buf, unsigned int size)
	{
		ASSERT(_is_init);
		TryServe(player,buf,size);
	}

	void Control(int param ,const void * buf, unsigned int size)
	{
		ASSERT(_is_init);
...
private:
	virtual bool OnServe(gplayer_imp *pImp, const XID & provider,const A3DVECTOR & pos, const void * buf, unsigned int size) = 0;

	virtual bool SendRequest(gplayer_imp *pImp, const XID & provider,const void * buf, unsigned int size) = 0;

public:
	bool Serve(gplayer_imp *pImp, const XID & provider,const A3DVECTOR & pos,const void * buf, unsigned int size)
	{
		return OnServe(pImp,provider,pos,buf,size);
	}

	bool ServeRequest(gplayer_imp *pImp, const XID & provider, const void * buf, unsigned int size)
	{
...
private:
	...
	{	
		...
		if(!(bRst1 && bRst2))
		{
			__PRINTINFO("service_manager:添加功能服务失败\n");
			throw 100;
		}
	}
