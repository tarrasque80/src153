...
/* Version Info */
unsigned int _task_templ_cur_version = 125;
...
inline bool LoadAwardDataBin(FILE* fp, AWARD_DATA& ad, unsigned int ulVersion)
{
	...
	for (i=0; i<ad.m_ulChangeKeyCnt; i++)
	{
		fread(&ad.m_plChangeKey[i], sizeof(int), 1, fp);
		fread(&ad.m_plChangeKeyValue[i], sizeof(int), 1, fp);
	...
	for (i=0; i<ad.m_ulHistoryChangeCnt; i++)
	{
		fread(&ad.m_plHistoryChangeKey[i], sizeof(int), 1, fp);
		fread(&ad.m_plHistoryChangeKeyValue[i], sizeof(int), 1, fp);
	...
	if (ad.m_ulDisplayKeyCnt)
	{
		...
		for (i=0; i<ad.m_ulDisplayKeyCnt; i++)
		{
			fread(&ad.m_plDisplayKey[i], sizeof(int), 1, fp);
		}
	}
...
bool ATaskTemplFixedData::LoadFixedDataFromBinFile(FILE* fp)
{
	...
	for (i=0; i<m_ulChangeKeyCnt; i++)
	{
		fread(&m_plChangeKey[i], sizeof(int), 1, fp);
		fread(&m_plChangeKeyValue[i], sizeof(int), 1, fp);
