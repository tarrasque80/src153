...
void ATaskTemplMan::ManualTrigTask(TaskInterface* pTask, unsigned int ulTask)
{
	_notify_svr(pTask, TASK_CLT_NOTIFY_MANUAL_TRIG, static_cast<unsigned short>(ulTask));
}

void ATaskTemplMan::ForceGiveUpTask(TaskInterface* pTask, unsigned int ulTask)
{
	_notify_svr(pTask, TASK_CLT_NOTIFY_FORCE_GIVEUP, static_cast<unsigned short>(ulTask));
}

void ATaskTemplMan::ForceRemoveFinishTask(TaskInterface* pTask, unsigned int ulTask)
{
#ifdef _DEBUG
...
void ATaskTemplMan::OnDynTasksTimeMark(TaskInterface* pTask, unsigned int ulTimeMark, unsigned short version)
{
...
void ATaskTemplMan::OnDynTasksData(TaskInterface* pTask, const void* data, unsigned int sz, bool ended)
{
	...
	unsigned int new_sz = sz + m_ulDynTasksDataSize;
...
void ATaskTemplMan::VerifyDynTasksPack(const char* szPath)
{
	...
	const unsigned int header_sz = sizeof(DYN_TASK_PACK_HEADER);
...
void ATaskTemplMan::OnTaskCheckAllTimeLimits(unsigned int ulCurTime)
{
}
...
void ATaskTemplMan::OnTaskRemoveFinishTask(TaskInterface* pTask, unsigned int ulTask)
{
...
void ATaskTemplMan::OnTaskUpdateStorage(TaskInterface* pTask, unsigned int ulCurTime)
{
	StorageTaskList* pLst = static_cast<StorageTaskList*>(pTask->GetStorageTaskList());
...
bool ATaskTemplMan::UpdateStorage(TaskInterface* pTask, StorageTaskList* pLst, unsigned int ulCurTime, unsigned int idStorage)
{
	bool result = false;
...
bool ATaskTemplMan::UpdateOneStorage(TaskInterface* pTask, StorageTaskList* pStorageLst, unsigned int _ulCurTime, int idStorage, bool init_refresh)
{
	//看时间是否过了0点，过了0点刷新次数清零，刷新时间更新
	time_t ulCurTime = _ulCurTime;
	if (idStorage == 0 || idStorage > TASK_STORAGE_COUNT) return false;
	struct tm* tm1 = localtime(&ulCurTime);
	unsigned int t_base = ulCurTime - tm1->tm_hour * 3600 - tm1->tm_min * 60 - tm1->tm_sec;
...
bool ATaskTemplMan::UpdateOneStorageDebug(TaskInterface* pTask, unsigned int _ulCurTime, int idStorage, bool bUseDayAsSeed)
{
	if (idStorage == 0 || idStorage > TASK_STORAGE_COUNT) return false;
	time_t ulCurTime = _ulCurTime;
	struct tm* tm1 = localtime(&ulCurTime);
	...
	if (pEssence != NULL) {
		...
		if (pEssence != NULL) {
			...
			memset(pStorageLst->m_Storages[index], 0, sizeof(unsigned short) * TASK_STORAGE_LEN);
			pStorageLst->m_StoragesRefreshTime[index] = ulCurTime;
			unsigned int randSeed = ((tm1->tm_year % 3) << 10) | tm1->tm_yday;
...
bool ATaskTemplMan::PickTaskByProbability(TaskInterface* pTask,StorageTaskList* pStorageLst, int idStorage, unsigned int ulCurTime, const NPC_TASK_OUT_SERVICE* pEssence)
{
	...
	unsigned int task_array_size = SIZE_OF_ARRAY(pEssence->id_tasks); 
	// 填充稀有任务容器和普通任务容器
	for (unsigned int j = 0; j < task_array_size; j++)
	{
	...
	unsigned int rand_count = pTask->RandNormal(min_count, max_count);
...
bool ATaskTemplMan::PickTaskByWeight(TaskInterface* pTask,StorageTaskList* pStorageLst, int idStorage, unsigned int ulCurTime, const TASK_DICE_BY_WEIGHT_CONFIG* pEssence, bool bDebug)
{
	...
	// 先按顺序根据每个任务集合的概率来添加
	unsigned int taskSetSize = SIZE_OF_ARRAY(pEssence->uniform_weight_list);
	for (unsigned int j = 0; j < taskSetSize && storageSubIndex < TASK_STORAGE_LEN; ++j, ++offsetIndex) {
	...
	if (totalWeight < pEssence->max_weight) {
		...
		if ( dt == DT_TASK_LIST_CONFIG && pConfig ) {
			unsigned int taskArraySize = SIZE_OF_ARRAY(pConfig->id_tasks); 
			for (unsigned int j = 0; j < taskArraySize; j++) {
		...
		unsigned int totalSize = vecTask.size();
		...
		unsigned int resultSize = vecResult.size();
		// 普通任务放到后面
		for (unsigned int  k = 0; k < resultSize && storageSubIndex < TASK_STORAGE_LEN; ++k) {
...
bool ATaskTemplMan::SaveTasksToPack(const char* szPackPath)
{
	...
	fseek(fp, sizeof(int) * tph.item_count, SEEK_CUR);

	int i = 0;
	for (unsigned int n = 0; n < TaskArr.size(); n++)
	{
	...
	fwrite(pOffs, sizeof(int), tph.item_count, fp);
...
HTREEITEM ATaskTemplMan::GetTopTreeItemByID(unsigned int ulID)
{
...
HTREEITEM ATaskTemplMan::GetTreeItemByID(unsigned int ulID)
{
...
struct ID_Descending:public binary_function<const unsigned int,const unsigned int,bool>
{
	bool operator()(const unsigned int ulID1,const unsigned int ulID2)const
	{
		return ulID1 > ulID2;
	}
};
...
bool ATaskTemplMan::SaveTasksToPack2(const char* szPackPath)
{
	...
	typedef std::set<unsigned int, ID_Descending> Set_ID;
	Set_ID IdSet;
...
bool ATaskTemplMan::CanGiveUpTask(unsigned int ulTaskId)
{
...
#ifndef _TASK_CLIENT
extern void OnTaskGiveUpOneTask(TaskInterface* pTask, unsigned int ulTaskId, bool bForce);
#endif
...
void ATaskTemplMan::OnForgetLivingSkill(TaskInterface* pTask)
{
	...
	for (unsigned int i = 0; i < m_SkillTaskLst.size(); i++)
	{
...
{
	lst.reserve(256);
	unsigned int count = m_TasksCanSeekOut.size();
...
void ATaskTemplMan::UpdateTasksSeekOutDiff(TaskInterface* pTask)
{
	...
	if (m_TasksCanDeliver.empty()) m_TasksCanDeliver.swap(ttl);
		...
		unsigned int count_before = m_TasksCanDeliver.size();
		unsigned int count_after = ttl.size();
...
{
	unsigned int count = m_TasksToPush.size();
	for (unsigned int i = 0; i < count; ++i) {
...
const NPC_TASK_OUT_SERVICE* ATaskTemplMan::GetStorageTaskEssence(unsigned int idStorage)
{
	...
	std::unordered_map<unsigned int, unsigned int>::iterator iter = m_StorageEssenseMap.find(idStorage);
...
const TASK_DICE_BY_WEIGHT_CONFIG* ATaskTemplMan::GetWeightTasksEssence(unsigned int idStorage)
{
	...
	std::unordered_map<unsigned int, unsigned int>::iterator iter = m_WeightEssenseMap.find(idStorage);
...
unsigned int ATaskTemplMan::GetStoragePremItemID(unsigned int idStorage)
{
	const NPC_TASK_OUT_SERVICE* pEssence = GetStorageTaskEssence(idStorage);
	unsigned int ret = 0;
...
bool ATaskTemplMan::InitStorageTask()
{
	...
	while (id) {
		if (dt == DT_NPC_TASK_OUT_SERVICE) {
			...
			unsigned int sTasks = SIZE_OF_ARRAY(pData->id_tasks);
			for (unsigned int i = 0; i < sTasks; i++) {
				if (pData->id_tasks[i]) {			
...
int ATaskTemplMan::LoadTasksFromPackForTimeTool(const char* szPackPath, bool bLoadDescript,unsigned int& lPackVersion,unsigned int& lToolVersion)
{
	...
	fread(pOffs, sizeof(int), tph.item_count, fp);
	
	for (unsigned int i = 0; i < tph.item_count; i++)
	{
...
ATaskTempl* ATaskTemplMan::GetTopTaskByID(unsigned int ulID)
{
...
ATaskTempl* ATaskTemplMan::GetTaskTemplByID(unsigned int ulID)
{
...
const ATaskTempl* ATaskTemplMan::GetProtectNPCTask(unsigned int ulNPCId)
{
...
void ATaskTemplMan::RemoveActiveStorageTask(StorageTaskList* pLst, unsigned int id)
{
...
void mount_task_out_service(const ATaskTempl* task, NPC_TASK_OUT_SERVICE* svr)
{
	...
	for (unsigned int i = 0; i < SIZE_OF_ARRAY(svr->id_tasks); i++)
	{
...
bool ATaskTemplMan::UnmarshalDynTasks(const char* data, unsigned int data_size, bool header_only)
{
...
bool ATaskTemplMan::UnmarshalNPCInfo(const char* data, unsigned int data_size, bool header_only)
{
