#ifndef __STORAGEWDB_H
#define __STORAGEWDB_H
...
public:
	...
	public:
		...
		void abort( ) { ThreadContext::GetInstance()->set_cancel(); }
	};
	...
	public:
		~Storage()
		{
			if(this)
			{
				...
				if(compressor) delete uncompressor;
				if(path) free(path);
			}
		}
		...
		void insert( const GNET::Octets &key, const GNET::Octets &val, Transaction &txn, int flags = 0 )
		{
			try
			{
				...
				{
					if ( ! db->put( key.begin(), (unsigned int)key.size(), com_val.begin(), (unsigned int)com_val.size(), false ) )
						throw DbException( DbException::WDB_OVERWRITE );
					new_key.insert( key );	
				}
				else if ( prepared.find ( key ) == prepared.end() && new_key.find(key) == new_key.end() )
				{
					unsigned int val_len = com_val.size();
					if ( void *origin_val = db->put( key.begin(), (unsigned int)key.size(), com_val.begin(), &val_len) )
...
public:
	...
	static void removeoldlogs( )
	{
		if( env )
		{
			std::vector<unsigned int> r = env->remove_logs();
			char *name = (char *)malloc( logdir.length() + 32 );
			std::vector<unsigned int>::iterator it, ite;
