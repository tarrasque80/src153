...
int GetNextVector( int type, const char * data, int len)
{
	switch(type)
	{
		case PLAYER_INFO_1_LIST:
		{
			...
			int offset = 0;
			//if(state & 0x2000) offset += 1;		//交通
			if(state & 0x1000) offset += 1;		//摆摊
			if(state & 0x0800) offset += 5;		//帮派
			if(state & 0x0400) offset += 8;		//广告
			if(state & 0x0001) offset += 1;		//变身
			if(state & 0x0002) offset += 1;		//表情
			if(state & 0x0040) offset += 4;		//扩展属性
			offset += 7;
			assert( offset <= len);
			offset += 4;
			if(offset < len )
