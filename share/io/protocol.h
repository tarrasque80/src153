...
public:
	...
	public:
		...
		protected:
			...
			Session(Manager *m) : sid(nextsid()), is(this), os_bytes(0), manager(m), state(m->GetInitState()), verbose(LOG_ERR),ignore(false){ }
		...
		private:
			...
			void OnRecv()
			{
				...
				try
				{
					int result = 0;
					Protocol* p;
					while ((p = Protocol::Decode(is)))
					{
						//получить результат нагрузки на gdeliveryd
						result = is.session->manager->LoadPolicy( sid , p->GetType(), p->GetSize() ); 
						
						switch (result)
						{
							case 0: //Всё хорошо, отправляем пакет
							{
								Protocol::Task::Dispatch(manager, sid, p); 
								continue;
							} 
							break; 
							case 1: //Превышен ООГ лимит, игнорируем тяжелые пакеты
							{ 
								continue;
							} 
							break;
							default: //Значительно превышен ООГ лимит, кикаем сессию
							{ 
	...
	public:
		...
		virtual bool CheckUnknownProtocol(Session::ID , Protocol::Type , unsigned int ) { return false; }
		virtual void DispatchUnknownProtocol(Session::ID , Protocol::Type , const OctetsStream & ) { }
		virtual int  LoadPolicy(Session::ID , Protocol::Type, unsigned int ) { return 0; }
	};
private:
	...
	static void CheckPolicy(Type type, unsigned int size, const Manager::Session::Stream &is)
	{
		if (Protocol::GetStub(type))
		{
			...
			if (!is.session->StatePolicy(type))
			{
				...
				{
					Log::log(is.session->GetVerbose(), "Protocol state or size policy error. sid=%d,type=%d,size=%d,acceptsize=%d",
...
public:
	...
	unsigned int GetSize() const { return iSize; }
