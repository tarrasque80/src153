...
class Compress
{
	...
	unsigned char* compress_block( unsigned char *obuf, size_t isize )
	{
		...
		*obuf = 0;
		while ( r - s > 2 )
		{
			unsigned char *p = GetPredecitAddr(s);
			if ( p >= s )
			{
...
public:
	Compress() : histptr(history), legacy_in(0) { memset (hash, 0, sizeof(hash));}
