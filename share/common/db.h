...
public:
	...
	void set_cache_peak( unsigned int peak )  { if ( peak > _cache_peak ) _cache_peak = peak; }
	void reset_peak()           	    { _cache_peak = 0, _dirty_peak = 0, _snapshot_peak = 0; }
	void set_cache_high ( unsigned int cache_high ) { _cache_high = cache_high; }
	void set_cache_low  ( unsigned int cache_low  ) { _cache_low  = cache_low ; }
	void set_dirty_peak ( unsigned int peak ) { if ( peak > _dirty_peak ) _dirty_peak = peak; }
	void set_snapshot_peak ( unsigned int peak ) { if ( peak > _snapshot_peak ) _snapshot_peak = peak; }
	unsigned int get_cache_high() const { return _cache_high; } 
	unsigned int get_cache_low () const { return _cache_low;  } 
	unsigned int record_count() const { return _insert - _remove_found; }
...
struct logger_hdr
{
	...
	unsigned int		logger_check_timestamp;
#define LCR_NULL	0
...
union PageLayout
{
	...
	union
	{
		struct
		{
			...
			unsigned int		logger_last_check;
			page_index_t	logger_first_idx;
...
inline void* extract_data( data_hdr  *hdr, unsigned int &size,    void *data_buf );
inline void* extract_key ( data_hdr  *hdr, unsigned int &key_len );
inline void* extract_key ( data_hdr  *hdr, unsigned int &key_len, void *key_buf  );
inline void* extract_val ( data_hdr  *hdr, unsigned int &val_len, void *val_buf  );
inline void* extract_val ( data_hdr  *hdr, unsigned int &val_len );
inline void* extract_data( index_hdr *hdr, unsigned int &size,    void *data_buf );
inline void* extract_key ( index_hdr *hdr, unsigned int &key_len );
inline void* extract_key ( index_hdr *hdr, unsigned int &key_len, void *key_buf  );
inline void* extract_val ( index_hdr *hdr, unsigned int &val_len, void *val_buf  );
inline void* extract_val ( index_hdr *hdr, unsigned int &val_len );
inline void  free_data   ( data_hdr  *hdr );
inline int compare( index_hdr *hdr, const void *key, unsigned int key_len);

class IQueryKey
{
public:
	virtual ~IQueryKey() { }
	virtual bool update( const void *key, unsigned int key_len ) = 0;
};
...
public:
	virtual ~IQueryData() { }
	virtual bool update( const void *key, unsigned int key_len, const void *val, unsigned int val_len ) = 0;
};
/*
inline unsigned int pread( int fd, void *buffer, unsigned int size, off_t off )
{
        lseek( fd, off, SEEK_SET );
        return read( fd, buffer, size );
}

inline unsigned int pwrite( int fd, const void *buffer, unsigned int size, off_t off )
{
        lseek( fd, off, SEEK_SET );
...
public:
	...
	index_hdr* find_key( const void *key, unsigned int key_len, int& pos )
	{
		performance()->find_key();
		if ( unsigned int size = index_size() )
	...
	index_hdr* find( const void *key, unsigned int key_len )
	{
		int pos;
	...
	index_hdr* great_equal_key( const void *key, unsigned int key_len )
	{
		int pos;
		index_hdr* hdr=find_key( key, key_len, pos ); 
		return ( hdr!=NULL && hdr==index_tail() ) ? NULL : hdr;
...
inline void* Page::operator new(unsigned int size) { return PagePool::alloc(); }
...
class PageHash
{
	...
	unsigned int  snapshot_size, snapshot_capacity;
	unsigned int	cache_high;
	unsigned int	cache_low;
	Page**  bucket;
	Page**  snapshot;
...
public:
	...
	void init( unsigned int high, unsigned int low )
	{
		unsigned int bucket_size = bucket_mask + 1;
	...
	Page** snapshot_reference(unsigned int& size) { size = snapshot_size; return snapshot; }

	unsigned int snapshot_create( Page *magic )
	{
	...
	Page* insert( Page *page )
	{
		...
		*it = page;
		...
		{
			unsigned int tmp_mask = ( (bucket_mask + 1) << 1 ) - 1;
...
class PageCache
{
	...
	PageHash     hash;
public:
	PageCache ( PageFile *file, unsigned int cache_high, unsigned int cache_low ) : page_file(file), magic(0, this)
	{
	...
	Page** snapshot_reference(unsigned int &snapshot_size) { return hash.snapshot_reference(snapshot_size); }
	...
	data_hdr* __alloc_data_head( unsigned int size )
	{
	...
	void adjust_data_head( data_hdr *hdr, unsigned int size )
	{
	...
	data_hdr* alloc_data_head( unsigned int size )
	{
	...
	data_hdr* alloc_data_head( data_hdr *pdr, unsigned int size )
	{
	...
	void create_node( index_hdr& index, const void *key, unsigned int key_len, const void *val, unsigned int val_len )
	{
		...
		unsigned int size      = key_len + val_len + sizeof(size32_t) + sizeof(size32_t);
		data_hdr *hdr    = alloc_data_head( size );
		unsigned int capacity  = hdr->size;
	...
	index_hdr* put_new_node( const void *key, unsigned int key_len, const void *val, unsigned int val_len )
	{
		int pos;
	...
	void* put_replace( index_hdr* found, const void *key, unsigned int key_len, const void *val, unsigned int& save_val_len, bool need_origin )
	{
		data_hdr *hdr        = found->Windex_hdr2data_hdr();
		unsigned int val_len       = save_val_len;
		void*  origin_val    = need_origin ? extract_val( hdr, save_val_len ) : NULL;
		unsigned int pass_len      = key_len + sizeof(size32_t) + sizeof(size32_t);
		unsigned int size          = pass_len + val_len;
		unsigned int capacity      = hdr->capacity();
	...
	bool put( const void *key, unsigned int key_len, const void *val, unsigned int val_len, bool replace )
	{
	...
	void* put( const void *key, unsigned int key_len, const void *val, unsigned int& val_len )
	{
		if ( index_hdr *found = put_new_node( key, key_len, val, val_len ) )
	...
	void walk( index_hdr *hdr, IQueryKey *query )
	{
		...
		{
			unsigned int key_len = sizeof(key_buf);
	...
	void walk( index_hdr *hdr, IQueryData *query )
	{
		...
		for ( bool r = true; r && hdr; hdr = Layout2Page(hdr)->next(hdr) )
		{
			unsigned int data_len = sizeof(data_buf);
			void*  data     = extract_data( hdr, data_len, data_buf );
			unsigned int key_len  = *((size32_t *)data + 1);
			unsigned int val_len  = *(size32_t *)data - key_len - sizeof(size32_t) - sizeof(size32_t);
			if ( key_len==0 ) 
			{
				if ( data != data_buf ) 
					::free(data);
	...
	template<typename T>
	void walk( const void *key, unsigned int key_len, T *query ) { walk ( root_index_page()->great_equal_key( key, key_len ), query ); }

	bool del( const void *key, unsigned int key_len )
	{
	...
	void* del( const void *key, unsigned int key_len, unsigned int& val_len )
	{
		if ( index_hdr *hdr = root_index_page()->find( key, key_len ) )
		{
			performance()->remove_found();
	...
	bool exist( const void *key, unsigned int key_len ) { return root_index_page()->find( key, key_len ); }
	unsigned int record_count() { return performance()->record_count(); }
};
...
public:
	...
	virtual State redo(unsigned int timestamp ) { return CLEAN; }
	virtual State prepare() { return PREPARED; }
	virtual State commit(unsigned int timestamp ) = 0;
...
public:
	...
	State commit(unsigned int timestamp )
	{
...
public:
	...
	State redo( unsigned int timestamp ) { return commit(timestamp); }
...
class GlobalLogger : public Logger
{
	...
	public:
		...
		void commit( PageFile *log_file, unsigned int _logger_id, unsigned int timestamp, page_index_t rec_index )
		{
			PageLayout *layout = (PageLayout *)PageMemory::alloc();
		...
		unsigned int init( PageFile *log_file, page_index_t page_index )
		{
			memset( key_page, 0, PAGESIZE );
			size32_t *lp = (size32_t *)key_page;
			unsigned int len = strlen( identity() ) + 1;
		...
		unsigned int& logger_id()          { return page_cache->magic_layout_ptr()->logger_id; }
		unsigned int& logger_last_check()  { return page_cache->magic_layout_ptr()->logger_last_check; }
	...
	unsigned int&       logger_chain()      { return key_page->logger_chain;      }
	...
	bool read_log_file( unsigned int new_time_stamp )
	{
	...
	void open_log_file( unsigned int new_time_stamp, int flag = O_CREAT|O_RDWR )
	{
		delete log_file;
	...
	bool load_log_file( unsigned int new_time_stamp )
	{
		bool r = true;
	...
	void init_log_file( unsigned int new_time_stamp )
	{
	...
	State restore( unsigned int timestamp, page_index_t rec_index )
	{
		try
		{
			unsigned int logger_id = get_logger_id();
			for ( LogInfoVec::iterator it = loginfo_vec.begin(), ie = loginfo_vec.end(); it != ie; ++it )
...
public:
	...
	GlobalLogger( const char *prefix, unsigned int rotate = 4096 ) : logdir( strdup(prefix) ), rotate_pages(rotate), 
	...
	State integrity_verify()
	{
		try
		{
			...
			for ( LogInfoVec::iterator it = loginfo_vec.begin(), ie = loginfo_vec.end(); it != ie; ++it )
			{
				unsigned int cur_logger_id = (*it)->logger_id();
				if ( cur_logger_id == 0 )
					need_rotate = true;
	...
	State redo( unsigned int timestamp )
	{
		try
		{
	...
	std::vector<unsigned int> check_version()
	{
		std::vector<unsigned int> r;
		LogInfoVec::iterator ii  = loginfo_vec.begin();
		unsigned int logger_last_check = (*ii)->logger_last_check();
		unsigned int logger_id         = (*ii)->logger_id();
	...
	std::vector<unsigned int> restore( unsigned int timestamp )
	{
		std::vector<unsigned int> r;
		LogInfoVec::iterator ii  = loginfo_vec.begin();
		unsigned int logger_last_check = (*ii)->logger_last_check();
	...
	std::vector<PageLayout *> read_magic( unsigned int timestamp, unsigned int start, const char *dbfile )
	{
	...
	unsigned int get_logger_id() const
	{
		time_t ts;
...
class PerformanceMonitor
{
	...
	unsigned int      perf_org_elapse;
	unsigned int      perf_cur_elapse;
public:
	...
	const Performance *performance_from_begin( unsigned int *t_org, unsigned int *t_new ) const { *t_org = time_org; *t_new = time_new; return &perf_org; }
	const Performance *performance_from_checkpoint( unsigned int *t_cur, unsigned int *t_new ) const { *t_cur = time_cur; *t_new = time_new; return &perf_cur; }
};
...
public:
	...
	LoggerMonitor( PerformanceMonitor *monitor, const char *dbfile, const char *logdir, unsigned int t_start = 0 ) : performance_monitor(monitor)
	{
		std::vector<unsigned int> r;
		...
		std::vector<unsigned int>::iterator it = std::lower_bound( r.begin(), r.end(), t_start );
		if ( it != r.end() )
		{
			if ( it != r.begin() )
				--it;
...
class PageBrowser
{
	...
	{
		unsigned int size = *(size32_t *)(hdr + 1);
...
public:
	...
	PageBrowser( const char *filename, unsigned int max = 1024 ) : file(new PageFile( filename, O_RDONLY ) ), cache_max(max)
	{
		hash.init( max, max );
	}

	unsigned int action( IQueryData *query )
	{
		int corrupt_count = 0;
		try
		{
...
class PageRebuild
{
	...
	public:
		...
		PageWriter( const char *dst, unsigned int max ) : 
		...
		unsigned int counter() const { return count; }
	};
...
public:
	...
	PageRebuild( const char *dst, const char *src, unsigned int max = 1024 ) : browser(new PageBrowser(src, max)), writer(new PageWriter(dst, max)) { }

	unsigned int action(unsigned int *corrupt_count)
	{
		unsigned int c = browser->action( writer );
...
inline unsigned int data_hdr::capacity() { return next - layout_offset(this) - sizeof(data_hdr); }
...
inline void* extract_data( data_hdr *hdr, unsigned int &size, void *data_buf )
{
	unsigned int copy_len = *(size32_t *)(hdr + 1);
...
inline void* extract_key( data_hdr *hdr, unsigned int &key_len )
{
	unsigned int copy_len = key_len = *((size32_t *)(hdr + 1) + 1);
	byte_t *key = (byte_t *)malloc( key_len );
	byte_t *p   = key;
	unsigned int min  = hdr->size - sizeof(size32_t) - sizeof(size32_t);
	if ( copy_len <= min )
...
inline void* extract_key( data_hdr *hdr, unsigned int &key_len, void *key_buf )
{
	unsigned int copy_len = *((size32_t *)(hdr + 1) + 1);
...
{
	unsigned int size     = *(size32_t *)(hdr + 1);
	unsigned int key_len  = *((size32_t *)(hdr + 1) + 1);
...
inline void* extract_val( data_hdr *hdr, unsigned int &val_len, void *val_buf )
{
	unsigned int size     = *(size32_t *)(hdr + 1);
	unsigned int key_len  = *((size32_t *)(hdr + 1) + 1);
	unsigned int copy_len = size - key_len - sizeof(size32_t) - sizeof(size32_t);
...
inline void* extract_data( index_hdr *hdr, unsigned int &size, void *data_buf   ) { return extract_data( hdr->Rindex_hdr2data_hdr(), size, data_buf   ); }
inline void* extract_key ( index_hdr *hdr, unsigned int &key_len ) { return extract_key ( hdr->Rindex_hdr2data_hdr(), key_len ); }
inline void* extract_key ( index_hdr *hdr, unsigned int &key_len, void *key_buf ) { return extract_key ( hdr->Rindex_hdr2data_hdr(), key_len, key_buf ); }
inline void* extract_val ( index_hdr *hdr, unsigned int &val_len ) { return extract_val ( hdr->Rindex_hdr2data_hdr(), val_len ); }
inline void* extract_val ( index_hdr *hdr, unsigned int &val_len, void *val_buf ) { return extract_val ( hdr->Rindex_hdr2data_hdr(), val_len, val_buf ); }
...
inline int memcmp_fast( const void *s1, const void *s2, unsigned int n )
{
...
inline int compare( index_hdr *hdr, const void *key, unsigned int key_len )
{
	if ( unsigned int i_key_len = hdr->key_len )
	{
	...
	unsigned int i_key_len = *((size32_t *)(ddr + 1) + 1);
	unsigned int cmp_len   = std::min( key_len, i_key_len );
	unsigned int min       = ddr->size - sizeof(size32_t) - sizeof(size32_t);
...
public:
	...
	bool put( const void* key, unsigned int key_len, const void* val, unsigned int val_len, bool replace = true )
	{
	...
	bool  del( const void* key, unsigned int key_len )  { return page_cache->del( key, key_len ); }
	void* del( const void* key, unsigned int key_len, unsigned int *val_len ) { return page_cache->del( key, key_len, *val_len ); }
	bool exist( const void* key, unsigned int key_len ) { return page_cache->exist( key, key_len ); }
	void walk ( IQueryKey  *query ) { page_cache->walk( query ); }
	void walk ( IQueryData *query ) { page_cache->walk( query ); }
	void walk ( const void *key, unsigned int key_len, IQueryKey  *query ) { page_cache->walk( key, key_len, query ); }
	void walk ( const void *key, unsigned int key_len, IQueryData *query ) { page_cache->walk( key, key_len, query ); }
	unsigned int record_count() const { return page_cache->record_count(); }
...
{
	unsigned int cache_high, cache_low;
	PageFile  *page_file;
	Logger    *logger;
public:
	~DBStandalone()
	...
	DBStandalone( const char *dbfile, unsigned int high = 1024, unsigned int low = 768 ) : cache_high( high ), cache_low( low ),
...
class DBCollection
{
	struct Table
	{
		...
		Table( const char *name, unsigned int high, unsigned int low ) : table_name(strdup(name)), cache_high(high), cache_low(low),
...
public:
	...
	DBCollection(unsigned int pages = 4096) : datadir(NULL), logdir(NULL), logpages(pages), logger(NULL) { }
	...
	void set_table ( const char *name, unsigned int cache_high, unsigned int cache_low )
	{
		table_vec.push_back ( new Table( name, cache_high, cache_low ) );
	}

	unsigned int load_tables()
	{
	...
	std::vector<unsigned int> version () { return logger->check_version(); }
	std::vector<unsigned int> restore (unsigned int timestamp) { return logger->restore( timestamp ); }
	std::vector<unsigned int> remove_logs()
	{
		std::vector<unsigned int> r;
		unsigned int time_stamp = logger->get_logger_id();
		if ( DIR *dir = opendir( logdir ) )
		{
			while ( dirent *d = readdir( dir ) )
				if ( d->d_name[0] != '.' )
