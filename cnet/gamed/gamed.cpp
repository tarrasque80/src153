...
void handle_user_msg(int cs_index,int sid, int uid, const void * msg, unsigned int size, char channel) {}
void user_kickout(int cs_index,int sid,int uid) {}
void user_lost_connection(int cs_index,int sid,int uid) {}
void user_login(int cs_index,int sid,int uid,const void * auth_buf, unsigned int size) {}
...
void switch_server_request(int link_id,int user_id, int localsid, int source,const void * , unsigned int ){}
...
namespace GDB
{
	unsigned int convert_item(const GNET::RpcDataVector<GNET::GRoleInventory> & ivec,  itemdata * list , unsigned int size) { return 0;}
}
