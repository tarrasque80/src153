...
public:
	void Init(const void* buf,unsigned int size)
	{
		Reset();
	...
	int GetBuf(void * buf, unsigned int size)
	{
		if(!size) return -1;
		unsigned char * pPrv = (unsigned char *)buf;
		unsigned int index = 0;
		for(unsigned int i= 0; i < PRIVILEGE_NUM; i ++)
		{
			if(privileges[i])
			{
				pPrv[index++] = i;
	...
	bool Has_NoTrade()
	{
		return privileges[PRV_NOTRADE]==1;
	}
	bool Has_NoAuction()
	{
		return privileges[PRV_NOAUCTION]==1;
	}
	bool Has_NoMail()
	{
		return privileges[PRV_NOMAIL]==1;
	}
	bool Has_NoFaction()
	{
		return privileges[PRV_NOFACTION]==1;
	}
	bool Has_DoubleMoney()
	{
		return privileges[PRV_DOUBLEMONEY]==1;
	}
	bool Has_DoubleObject()
	{
		return privileges[PRV_DOUBLEOBJECT]==1;
	}
	bool Has_DoubleSP()
	{
		return privileges[PRV_DOUBLESP]==1;
	}
	bool Has_NoSellPoint()
	{
		return privileges[PRV_NOSELLPOINT]==1;
	}
};//end of class
