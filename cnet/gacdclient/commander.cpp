...
//	bool first =true;
	...
	for(size_t jj = 0; jj <1; ++jj)
	{
		...
		{
			ACQuery acql;
			acql.acq.query_type = (cmd == "logs" ? AC_QUERY_LOGINOUT : AC_QUERY_STR );
			acql.acq.roleid = atoi(cmds[2].c_str());
		...
		{
			ACQuery acql;
			acql.acq.query_type = AC_QUERY_WHO;
			GAntiCheaterClient::SendProtocol( acql, atoi(cmds[1].c_str()));
			return;
		}
		...
		{
			ACQuery acql;
			acql.acq.query_type = AC_QUERY_FORBID_PROCESS;
			GAntiCheaterClient::SendProtocol( acql, atoi(cmds[1].c_str()));
			return;
		}
		...
		{
			ACQuery acql;
			acql.acq.query_type = AC_QUERY_BRIEF_INFO;
			GAntiCheaterClient::SendProtocol( acql, atoi(cmds[1].c_str()));
			return;
		}
		...
		{
			ACQuery acql;
            acql.acq.query_type = AC_QUERY_PATTERN;
            acql.acq.roleid = -1; 
            int t_aid = 0;
            if( cmds.size() > 1 ) t_aid = atoi(cmds[1].c_str());
            if( cmds.size() > 2 ) acql.acq.roleid = atoi(cmds[2].c_str());
		...
		{
			ACQuery acql;
            acql.acq.query_type = AC_QUERY_PERIOD_INFO;
            acql.acq.roleid = -1; 
            int t_aid = 0;
            if( cmds.size() > 1 ) t_aid = atoi(cmds[1].c_str());
            if( cmds.size() > 2 ) acql.acq.roleid = atoi(cmds[2].c_str());
		...
		{
			ACQuery acql;
			acql.acq.query_type = AC_QUERY_PLATFORM_INFO;
			acql.acq.roleid = -1;
			int t_aid = 0;
			if( cmds.size() > 1 ) t_aid = atoi(cmds[1].c_str());
			if( cmds.size() > 2 ) acql.acq.roleid = atoi(cmds[2].c_str());
...
{   
    ACQuery query;
    query.acq.query_type = AC_QUERY_CHEATER;
...
{
	ACQuery query;
	query.acq.query_type = AC_QUERY_PATTERN;
    GAntiCheaterClient::SendProtocol(query, 0);
}
...
void Commander::TimerUpdate()
{
	...
	if( ++tick %4 == 0 )
	{
		...
		if( share_p && *share_p == 1023 )
		{
			...
			ACQuery query;
			query.acq.query_type = AC_QUERY_VERSION;
			GAntiCheaterClient::SendProtocol(query, 0);
			*share_p = 116;
			printf("haha\n");
		}
